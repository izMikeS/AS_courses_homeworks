...
namespace OnlineStore.WebPLL.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(ICustomerService c)
        {
            _customerService = c;
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    await UserManager.AddToRoleAsync(user.Id, RolesConstants.CUSTOMER);

                    var registeredUser = await UserManager.FindByNameAsync(model.Email);

                    _customerService.Create(new BLL.Dtos.Models.CustomerDto()
                    {
                        Email = user.Email,
